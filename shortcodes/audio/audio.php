...
if ( ! class_exists( 'IG_Audio' ) ) {
	...
	class IG_Audio extends IG_Pb_Shortcode_Element {
			...
			if ( file_exists( ABSPATH . 'wp-includes/js/mediaelement/wp-mediaelement.js' ) ) {
				...
				if ( is_admin() ) {
					if ( file_exists( ABSPATH . 'wp-includes/js/mediaelement/mediaelement-and-player.min.js' ) ) {
						wp_register_script( 'ig-pb-mediaelement', get_site_url() . '/wp-includes/js/mediaelement/mediaelement-and-player.min.js' );
					} else {
						wp_register_script( 'ig-pb-mediaelement', get_site_url() . '/wp-includes/js/mediaelement/mediaelement-and-player.js' );
					}

					wp_enqueue_script( 'ig-pb-mediaelement' );
				} else {
		...
		/**
		 * Method to check if file existed on SounCloud
		 */
